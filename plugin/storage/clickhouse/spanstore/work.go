...
func (worker *WriteWorker) Work() {
	...
	if err := worker.writeBatch(worker.batch); err != nil {
		worker.params.logger.Error("Could not write a batch of spans", zap.Any("error", err), zap.Any("worker_id", worker.workerID))
	} else {
		worker.close()
		return
	}
...
func (worker *WriteWorker) writeBatch(batch []*model.Span) error {
	worker.params.logger.Debug("Writing spans", zap.Any("size", len(batch)))
